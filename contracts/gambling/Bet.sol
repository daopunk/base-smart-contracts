...
contract Bet {
	...
	function getScoreDifference(Game.Teams team) view public returns (int) {
		if(team == Game.Teams.Team1) {
	...
	function placeBet(Game.Teams teamNumber) external payable returns(uint) {
		return calculatePayout(msg.value, getScoreDifference(teamNumber));
	}
}